/**
 * node-dirty - a tiny append-only key-value store.
 *
 * Vendored from https://github.com/felixge/node-dirty
 * Copyright (c) 2010 Debuggable Limited <felix@debuggable.com>
 * MIT License - see LICENSE.node-dirty.txt in this directory.
 *
 * Stores JSON records as newline-delimited rows in a file. The full dataset
 * is loaded into memory on construction; writes append to disk. Keys are
 * mutable in memory (last write wins) and the file is an append-only log.
 *
 * Used by zygos as the backing store for the encrypted askpass cache.
 * The passphrases themselves are AES-256-GCM encrypted before storage; this
 * file contains only ciphertext and metadata.
 */
'use strict';
